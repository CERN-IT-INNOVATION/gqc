...
class AE_sinkhorn(AE_vanilla):
    ...
    def print_losses(epoch, epochs, train_loss, valid_losses):
        ...
        print(f"Epoch : {epoch + 1}/{epochs}, "
              f"Valid sinkh loss = {valid_losses[2].item():.8f}")
    ...
    def valid(self, valid_loader, outdir):
        ...
        laten_loss = self.laten_loss_weight * \
            self.laten_loss_function(latent, latent_noise)

        valid_loss = laten_loss + recon_loss
    ...
    def train_all_batches(self, train_loader):
        ...
        for batch in train_loader:
            ...
            y_batch          = self.transform_target_data(y_batch)
