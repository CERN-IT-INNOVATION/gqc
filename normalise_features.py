...
def normalizerMeanStd(bkgfile,sigfile,outfile,fileflag):
	...
	meanBkg, meanSig = np.mean(bkg[:ntrain],axis=0),np.mean(sig[:ntrain],axis=0)
...
if __name__ == "__main__":
	...
	bkgNorm,sigNorm = normalizerMinMax(infileBkg,infileSig,outfile,args.fileFlag)
	bkgNormStd,sigNormStd = normalizerMeanStd(infileBkg,infileSig,outfile,args.fileFlag)
