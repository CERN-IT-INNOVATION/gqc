...
class AE(nn.Module):
    ...
    def __init__(self, node_number, dropout=False, **kwargs):
        ...
        self.encoder_layers = construct_layers(True)
        self.encoder = nn.Sequential(*self.encoder_layers)

        self.decoder_layers = construct_layers(False)
        self.decoder = nn.Sequential(*self.decoder_layers)

    def construct_layers(self, encoder):
    	"""
    	...
    	layers = []
    	if encoder: nodes = range(self.node_number)
    	else: nodes = reverse(range(self.node_number))

        for idx in nodes:
            if dropout == True:
                if idx != 0: prob=0.2; layers.append(nn.Dropout(p=prob))

            layers.append(nn.Linear(node_number[idx], node_number[idx+1]))

            if encoder and idx == len(node_number)-2: break
...
def eval_valid_loss(model, valid_loader, criterion, min_valid, outdir):
...
def eval_train_loss(batch_features, device, criterion, optimizer):
...
def train(train_loader, valid_loader, model, criterion, optimizer, epochs,
	...
	for epoch in range(epochs):
		...
		for i, batch_features in enumerate(train_loader):
			train_loss =
				eval_train_loss(batch_features, device, criterion, optimizer)

		valid_loss =
			eval_valid_loss(model, valid_loader, criterion, min_valid, outdir)
