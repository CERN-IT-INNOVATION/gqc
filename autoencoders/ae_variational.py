...
class AE_variational(AE_vanilla):
    ...
    @staticmethod
    def construct_encoder(layers, en_activ=None) -> nn.Sequential:
        """
        ...
        for idx in layer_nbs:
            if idx == len(layers) - 2 and en_activ is None:
                break
            if idx == len(layers) - 2:
    ...
    def reparametrize(self, mu, log_var) -> list:
        """
    ...
    def compute_loss(self, x_data, y_data) -> float:
        ...
        Combine the reconstruction loss with the KL divergence loss to
        get the total loss which is then propagated backwards..
        ...
        comp_probs = func.softmax(samples, dim=1).to(self.device)
