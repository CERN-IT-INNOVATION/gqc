...
parser.add_argument("--maxdata", type=int, default=-1,
                    help="The maximum train samples to use for signal. "
                         "bkg will be equal.")
...
def split_and_save(data, target, name):
    ...
    x_train, x_valid, y_train, y_valid = \
        train_test_split(data, target, test_size=args.valid_percent,
                         shuffle=True, stratify=target)
    test_percent = float(x_valid.shape[0]/x_train.shape[0])
    x_train, x_test, y_train, y_test = \
        train_test_split(x_train, y_train, test_size=test_percent,
