...
def ratio_plotter(input_data, output_data, ifeature, class_label=''):
    ...
    plt.xlabel(varname(ifeature) + ' (normalized)')
    plt.ylabel('Density')
    plt.xlim(0, 0.4)
    plt.legend()
...
def sig_bkg_plots(input_sig, input_bkg, output_sig, output_bkg, model_path):
    ...
    for idx in range(input_sig.numpy().shape[1]):
        ...
        plt.savefig(model_path + 'ratio_plotter' + varname(idx) + '.pdf')
        plt.clf()
...
def diagnosis_plots(loss_train, loss_valid, min_valid, nodes, batch_size, lr):
    ...
    plt.ylabel("MSE")
    plt.xlabel("epochs")
    plt.title("B = " + str(batch_size) + ", lr=" + str(learning_rate) + ", " +
...
def mape(output, target, epsilon=1e-4):
    # A different kind of criterion... Not sure what though.
...
if __name__ == '__main__':
    ...
    model = load_model(AE, args.layers, args.model)
