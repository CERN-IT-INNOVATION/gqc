...
class AE_sinkclass(AE_vanilla):
    ...
    def construct_noise_gen_input(self):
        """
        Construct the input layers to the noise generator. The raw data
        is fed to a layer and then reshaped to a dimension compatible
        to the noise generator. The same is done for the target data.
        """
    ...
    def generate_noise(self, x, y) -> np.ndarray:
        """
    ...
    def valid(self, valid_loader, outdir) -> float:
        ...
        returns :: Pytorch loss object of the total validation loss,
            latent loss on the validation data, recon loss on it, and
            the classifier loss on it.
        """
    ...
    def predict(self, x_data) -> np.ndarray:
        ...
        returns :: The latent space, reconstructed data, and the
            classifier output of the ae.
        """
        x_data = torch.from_numpy(x_data).to(self.device)
        self.eval()
        latent, classif, recon = self.forward(x_data.float())
